...
const sum = numberList.reduce((count,x)=>{return count+x},0);
  return callback(sum) 
}
...
function processDuplicateFree(list,callback) {
  const deduped = list.reduce((unique,item)=>{
    const duplicates = unique.includes(item); 
    if(!duplicates) 
    {unique.push(item);
    } 
     return unique;
...
function tallyUpDonations(runners) {
  return runners.reduce((runners,sum)=>{return runners.donation + sum},0);
